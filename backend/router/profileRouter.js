...
/*
    @usage : Follow/Unfollow A User with User1Id
    @url : /api/profiles/follow/:followedUserId
    @fields : no-fields
    @method : PUT
    @access : PRIVATE
 */
    router.put("/follow/:followedUserId", authenticate, async (request, response) => {
      try {
        // console.log("router console");
        let followedUser = request.params.followedUserId;     //user to be followed
        var followerUser = request.user.id;           //user who fill follow
    
        let followerUserProfile = await User.findOne({ _id: followerUser });    //finding profile of user who will follow
        // console.log(followerUserProfile);
        var isFollowed = followerUserProfile.following.includes(followedUser);    //checking if user already follows
    
        var option = isFollowed ? "$pull" : "$addToSet";
        // Insert/delete post like
        let profile = await User.findOneAndUpdate(
          { _id: followerUser },
          { [option]: { following: followedUser } },
          { new: true }
        );
        let profile1 = await User.findOneAndUpdate(
          { _id: followedUser },
          { [option]: { followers: followerUser } },
          { new: true }
        );
        response.status(200).json({ profile: profile,profile1:profile1 });
      } catch (error) {
        console.error(error);
        response.status(500).json({ errors: [{ msg: error.message }] });
      }
